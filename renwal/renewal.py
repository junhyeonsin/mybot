...
con = pymysql.connect(user=os.environ['user'], password=os.environ['password'],
                      host=os.environ["host"], charset="utf8", database=os.environ["database"], connect_timeout=120)
...
class MyClient(discord.Client):
    ...
    async def on_message(self, message: discord.Message):
        ...
        if emoji == f":{r}:":
            ...
            cur.execute("SELECT ONOFF FROM onoff WHERE GUILD = %s",
                        (str(guild.id)))
            check = cur.fetchone()
            if not check:
                cur.execute("INSERT INTO onoff VALUES(%s,%s)",
                            (str(guild.id), True))
                con.commit()
                check = (True, 0)
...
class reportModal(ui.Modal, title="건의"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        cur.execute("INSERT INTO report VALUES(%s,%s)",
                    (self.answer.value, interaction.user.id))
        con.commit()
...
async def emojionoff(interaction: Interaction, 온오프: onoff):
    if interaction.user.guild_permissions.manage_messages:
        cur = con.cursor()
        cur.execute(
            "CREATE TABLE IF NOT EXISTS onoff(GUILD INTEGER PRIMARY KEY,ONOFF BOOL)")
        cur.execute("SELECT * FROM onoff WHERE GUILD = ?",
                    (interaction.guild.id,))
        check = cur.fetchone()
        if not check:
            print(check)
            cur.execute("INSERT INTO onoff VALUES(?,?)",
                        (interaction.guild.id, True,),)
            con.commit()
        cur.execute(
            f"UPDATE onoff SET ONOFF = {온오프.value}  WHERE GUILD={interaction.guild.id}")
        con.commit()
...
def nextsong(interaction: Interaction, error):
    ...
    if not len(queue[guild]) == 0:
        voice_client: discord.VoiceClient = discord.utils.get(
            client.voice_clients, guild=interaction.guild)
        voice_client.play(
            queue[guild][0], after=lambda e: nextsong(interaction, e))
...
async def register(interaction: discord.Interaction, 닉네임: str):
    ...
    cur.execute("INSERT INTO user_data VALUES(%s, %s, %s, %s, %s , %s, %s)", (닉네임,
                interaction.user.id, 1, 0, 500, 0, str(interaction.user.display_avatar.url)))
    cur.execute("INSERT INTO user_stat VALUES(%s, %s, %s, %s, %s ,%s ,%s, %s, %s, %s)",
                (닉네임, interaction.user.id, 1, 1, 1, 1, 1, 0, 3, 1))
    con.commit()
...
async def info(interaction: discord.Interaction, 유저: discord.Member):
    ...
    async def button_callback(interaction: discord.Interaction):
        ...
        cur.execute(
            "SELECT name,str,dex,`int`,luck,hp,mp,stat_point,skill_point FROM user_stat WHERE id =%s", (유저.id))
        check = cur.fetchone()
        stat = ['닉네임', '힘', '민첩', '지능', '행운', '체력',
                "마나", '남은 스테이터스 포인트', '남은 스킬 포인트']
    ...
    async def info_callback(interaction: discord.Interaction):
        ...
        embed.set_thumbnail(url=유저.avatar)
        cur.execute(
            "SELECT name,level,exp,money,class,url FROM user_data WHERE id = %s", (유저.id))
        check = cur.fetchone()
        display = ["닉네임", "레벨", "", "돈", "", ""]
        for i in range(len(check)):
            if i == 2:
                ...
                block_id = ["0_", "1_", "2_", "3_", "4_",
                            "5_", "6_", "7_", "8_", "9_", "10", "9_5"]
                block_list = [discord.utils.get(
                    guild.emojis, name=i) for i in block_id]
                embed.add_field(name=f"경험치 {exp.string()} ({round(exp.percent(),1)}%)", value=exp.block(
                    block_list), inline=False)
            elif i == 4:
                transfer = Class(check[i])
                embed.add_field(
                    name="직업", value=transfer.display(), inline=True)
            elif i == 5:
                pass
            else:
...
async def status(interaction: discord.Interaction, 스텟: Status, 포인트: int):
    cur = con.cursor()
    cur.execute("SELECT stat_point,hp FROM user_stat WHERE id = %s",
                (interaction.user.id))
    check = cur.fetchone()
    if check[0] == 0:
        title = "스테이터스 에러"
        ...
        cur.execute(
            f"UPDATE user_stat SET {스텟.value}={스텟.value}+{check[0]},stat_point=stat_point-{check[0]} WHERE id = %s", (interaction.user.id))
        con.commit()
    elif 포인트 < 0:
        title = "스테이터스 에러"
        ...
        cur.execute(
            f"UPDATE user_stat SET {스텟.value} = {스텟.value}+{포인트},  stat_point=stat_point-{포인트}  WHERE id = %s", (interaction.user.id))
        con.commit()
...
async def reinforcement(interaction: discord.Interaction, 장비: ReinforceItem):
    ...
    if 장비.value != 0:
        cur.execute(
            f"SELECT * FROM `{interaction.user.id}_wear` WHERE part = %s AND wear = %s ", (장비.value, 1))
    else:
        cur.execute(
            f"SELECT * FROM `{interaction.user.id}_weapon` WHERE wear = %s", (1))
    check = cur.fetchone()
    if not check:
        ...
        view = ui.View()
        cur.execute(
            f"SELECT item_amount FROM `{interaction.user.id}_etc` WHERE item_code = %s", (1))
        amount = cur.fetchone()
        if amount == None:
            amount = 0
        else:
            amount = amount[0]
        cur.execute("SELECT money FROM user_data WHERE id = %s",
                    (interaction.user.id))
        gold = cur.fetchone()[0]
        if check[2] == "F":
        ...
        embed.add_field(
            name="재료", value=f"{a}골드 , 강화의 서 {int(a/50)}개", inline=False)
        ...
        if 장비.value != 0:
            select.options.append(SelectOption(
                label="체력", value="hp", description="캐릭터의 추가 체력"))
        select.options.append(SelectOption(
            label="마나", value="mp", description="캐릭터의 추가 마나"))
        view.add_item(select)

        async def select_callback(interaction: discord.Interaction):
            cur.execute(
                f"UPDATE user_data SET money = money -{a} WHERE id = %s", (interaction.user.id))
            cur.execute(
                f"UPDATE `{interaction.user.id}_etc` SET item_amount=item_amount - {int(a/50)} WHERE item_code=1")
            con.commit()
            if rein.rein():
                ...
                if 장비.value == 0:
                    cur.execute(
                        f"UPDATE `{interaction.user.id}_weapon` SET upgrade = upgrade+1, {select.values[0]}={select.values[0]}+{r} WHERE wear = 1")
                else:
                    cur.execute(
                        f"UPDATE `{interaction.user.id}_wear` SET upgrade = upgrade+1, {select.values[0]}={select.values[0]}+{r} WHERE wear = 1 AND part= %s", (장비.value))
                con.commit()
            else:
                title = "강화실패"
                name = "아쉽지만 다음기회에"
...
async def dungeon(interaction: discord.Interaction, 층: int):
    async def callback(interaction: Interaction):
        await interaction.response.send_message("준비중이에요!", ephemeral=True)
        cur = con.cursor()
        ...
        def em():
            ...
            embed.add_field(name=f"{damage.display()}⚔",
                            value="\u200b", inline=True)
            return embed

        def vi():
            view = ui.View()
            attack = ui.Button(style=discord.ButtonStyle.green,
                               emoji="⚔", label="공격하기")
            use = ui.Button(style=discord.ButtonStyle.gray,
                            emoji="💊", label="아이템")
            guard = ui.Button(style=discord.ButtonStyle.gray,
                              emoji="🔮", label="스킬사용", disabled=skill.canskill())
            run = ui.Button(style=discord.ButtonStyle.red,
                            emoji="👟", label="도망가기", disabled=True)
        ...
        async def item_select_callback(interaction: discord.Interaction):
            embed = em()
            cur.execute(
                f"SELECT item_code,item_name,item_amount,trade FROM `{interaction.user.id}_use` WHERE item_code BETWEEN 1 AND 50 AND item_amount != 0")
            getItem = cur.fetchall()
            view = ui.View()
            select = ui.Select(placeholder="아이템 사용", options=[
                               SelectOption(label="돌아가기", value=-1)])
            for i in range(len(getItem)):
                select.options.append(SelectOption(
                    label=f"{getItem[i][1]} {getItem[i][2]}개", value=getItem[i][0], description=("회복 아이템")))
            view.add_item(select)
        ...
        async def skill_select_callback(interaction: discord.Interaction):
            embed = em()
            cur.execute(
                f"SELECT skill_name,skill_mana,skill_hp FROM `{interaction.user.id}_skill` WHERE skill_level != 0 ")
            getSkill = cur.fetchall()
            view = ui.View()
            select = ui.Select(placeholder="스킬 사용", options=[
                               SelectOption(label="돌아가기", value=-1)])
            for i in range(len(getSkill)):
                if mp < getSkill[i][1] or hp < getSkill[i][2]:
                    pass
                else:
                    select.options.append(SelectOption(label=getSkill[i][0], value=i, description=(
                        f"마나소모:{getSkill[i][1]}" if getSkill[i][1] else "" + f"체력소모:{getSkill[i][2]}" if getSkill[i][2] else "")))
            view.add_item(select)
            ...
            async def skill_damage_callback(interaction: discord.Interaction):
                if int(select.values[0]) == -1:
                    ...
                    enemy = list(enemy)
                    cur.execute(
                        f"SELECT * FROM `{interaction.user.id}_skill` LIMIT {select.values[0]},1")
                    info = cur.fetchone()
                    premyhp = hp
                    ...
                    for key in effect.keys():
                        ...
                        hp, dam, enemy[1], enemy[2], value = skill.fighteffect(
                            key, hp, damage.display(), enemy[1], enemy[2])
                        emojis[value] = emoji
                        if effect[key] == 0:
                            li.append(key)
                    for i in li:
                    ...
                    if not value == "stun":
                        embed.add_field(
                            name=f"받은 데미지 {enemy[2]}💔", value='\u200b', inline=False)
                    embed.add_field(
                        name=f"준 데미지 {info[4]+ (damage.display()*info[5])}🗡", value="\u200b", inline=False)
                    if preenemyhp > enemy[1]+info[4] + (damage.display()*info[5]):
                        embed.add_field(
                            name=f'준 데미지 {preenemyhp-(enemy[2]+info[4]+ (damage.display()*info[5]))}{emojis[value]}', value="\u200b", inline=False)
        ...
        async def attack_callback(interaction: discord.Interaction):
            ...
            if dam != 0 and value == "damage":
                embed.add_field(
                    name=f'준 데미지 {dam}{emojis[value]}', value="\u200b", inline=False)
            await end(interaction)
            if not interaction.response.is_done():
        ...
        async def end(interaction: discord.Interaction):
            ...
            if enemy[1] <= 0:
                ...
                button = ui.Button(
                    label="다시 탐험", style=ButtonStyle.green, disabled=False)
                ...
                if level:
                    embed.add_field(
                        name=f"**{level} 레벨** 달성!", value="\u200b", inline=False)

                if rewardWeapon:
                    embed.add_field(
                        name=f"[무기] **{rewardWeapon}**를 획득했습니다.", value='\u200b', inline=False)
                if rewardWear:
                    embed.add_field(
                        name=f"[장비] **{rewardWear}**를 획득했습니다.", value='\u200b', inline=False)
                embed.set_thumbnail(
                    url="https://cdn.discordapp.com/attachments/988424121878741022/989064226083590145/chest.png")
                embed.add_field(
                    name=f"**{enemy[4]}골드 {enemy[5]}경험치**를 획득했습니다.", value="\u200b", inline=False)
                for i in range(len(etcname)):
                    embed.add_field(
                        name=f"[기타] **{etcname[i]} {etcamount[i]}개**를 획득했습니다.", value='\u200b', inline=False)
                for i in range(len(usename)):
                    embed.add_field(
                        name=f"[소비] **{usename[i]} {useamount[i]}개**를 획득했습니다.", value='\u200b', inline=False)
                await interaction.response.edit_message(embed=embed, view=view)
        if message_dic[str(interaction.user.id)]:
            await interaction.response.edit_message(embed=em(), view=vi())
        else:
            await interaction.edit_original_response(embed=em(), view=vi(), ephemeral=True)
    ...
    async def inventory_callback(interaction: discord.Interaction):
        ...
        if inventory.value == "_weapon" or inventory.value == "_wear":
            ...
            options = [SelectOption(label=(
                f"[{item[i][0]}] Lv.{item[i][4]} {item[i][3]} +{item[i][2]} {item[i][1]} {var if item[i][14+a.index(inventory.value)] else empty}"), value=i) for i in range(len(item))]
        else:
            var = "거래가능"
            val = "거래불가"
            options = [SelectOption(label=(
                f"({item[i][0]}) {item[i][1]} ({var if item[i][4] else val})"), value=i)for i in range(len(item))]
        ...
        async def equip_callback(interaction: discord.Interaction):
            ...
            if inventory.value == "_weapon":
                cur.execute(
                    f"UPDATE `{interaction.user.id}{inventory.value}` SET wear = 0 WHERE wear = 1")
                cur.execute(
                    f"UPDATE `{interaction.user.id}{inventory.value}` SET wear = 1 WHERE item_code = {item[int(select.values[0])][0]}")
            elif inventory.value == "_wear":
                cur.execute(
                    f"UPDATE `{interaction.user.id}{inventory.value}` SET wear = 0 WHERE wear = 1 AND part = {item[int(select.values[0])][16]}")
                cur.execute(
                    f"UPDATE `{interaction.user.id}{inventory.value}` SET wear = 1 WHERE item_code = {item[int(select.values[0])][0]}")
            con.commit()
            embed.set_footer(text="성공적으로 아이템을 착용했습니다.")
            view = ui.View()
            back = ui.Button(style=ButtonStyle.primary,
                             emoji="↩", label="돌아가기")
            view.add_item(back)
        ...
        async def select_callback(interaction: discord.Interaction):
            info, true, url, tem, gap = inventory.display(
                item[int(select.values[0])])
            ...
            if inventory.value == "_weapon" or inventory.value == "_wear":
                for i in range(len(tem)):
                    if i == 0:
                        ...
                        embed.add_field(
                            name=info[i], value=f"{tem[i]} {gap[i] if gap[i] else empty}", inline=true[i])
                cur.execute(
                    "SELECT level FROM user_data WHERE id = %s", (interaction.user.id))
                level = cur.fetchone()[0]
                equip = ui.Button(style=ButtonStyle.green, emoji="🛡", disabled=(
                    False if level >= tem[3] else True), label=("착용하기" if level >= tem[3] else "레벨이 낮습니다."))
                back = ui.Button(style=ButtonStyle.primary,
                                 emoji="↩", label="돌아가기")
                view.add_item(equip)
                view.add_item(back)
...
async def 스킬(interaction: discord.Interaction):
    ...
    for i in range(len(skill.select())):
        select.options.append(SelectOption(label=(
            f"(Lv. {skill.select()[i][3]}) {skill.select()[i][0]} ({skill.select()[i][1]}/{skill.select()[i][2]})"), value=i))
    view.add_item(select)
    ...
    def func():
        ...
        name = ['스킬명', "", '마나소모량', '체력소모량', '스킬데미지', '계수', '효과',
                '지속시간', '직업', "", '스킬포인트', '스킬레벨', "최대레벨", "요구레벨"]
        ...
        cur.execute("SELECT skill_point FROM user_stat WHERE id = %s",
                    (interaction.user.id))
        point = cur.fetchone()[0]
        print(point)
        embed.set_footer(text=f"남은 스킬포인트: {point}")
        view = ui.View()
        button = ui.Button(style=ButtonStyle.green, label="스킬레벨업⬆",
                           disabled=skill.require(select.values[0]))
        view.add_item(button)
        button.callback = button_callback
        return embed, view
    ...
    if sure:
        ...
        async def button_callback(interaction: discord.Interaction):
            cur.execute("DELETE FROM user_stat WHERE id = %s ",
                        (interaction.user.id))
            cur.execute("DELETE FROM user_data WHERE id = %s ",
                        (interaction.user.id))
            li = ["weapon", "wear", "etc", "use", "cash"]
            for i in li:
                cur.execute(
                    f"DELETE FROM trade_{i} WHERE id = %s", (interaction.user.id))
            con.commit()
            li.append("skill")
            for i in li:
...
async def makeitem(interaction: Interaction, 종류: mkItem):
    ...
    for i in item:
        if 종류.name == "무기" or 종류.name == "방어구":
            ...
            val = ["거래가능", "거래불가"]
            select.add_option(
                label=f"{i[1]} ({val[0] if i[3] else val[1]})", value=i[1])
    view = ui.View()
    view.add_item(select)
    ...
    async def button_callback(interaction: Interaction):
        ...
        cur.execute(
            f"SELECT url FROM make{종류.value} WHERE item_name = %s", item_global)
        embed = discord.Embed(title="아이템 제작 결과")
        embed.add_field(
            name=f"[{종류.name}] {item_global} {am}개 제작성공", value="\u200b")
...
#            print(item_global)
    ...
    def em(item, am):
        ...
        cur.execute(
            f"SELECT url FROM make{종류.value} WHERE item_name = %s", item)
        url = cur.fetchone()[0]
        need_etc, need_etc_amount, etc_amount, need_use, need_use_amount, use_amount = make.callamount(
            종류.name, interaction.user.id, item)
        for i in range(len(need_etc)):
            cur.execute(
                f"SELECT item_name FROM etc WHERE item_code = {need_etc[i]}")
            embed.add_field(
                name=f"{cur.fetchone()[0]}\n{int(need_etc_amount[i])*am}개\n보유중 : ({etc_amount[i]})", value="\u200b")
        for i in range(len(need_use)):
            cur.execute(
                f"SELECT item_name FROM `use` WHERE item_code = {need_use[i]}")
            embed.add_field(
    ...
    def vi(item):
        button = ui.Button(style=ButtonStyle.green, label="제작하기", disabled=make.disable(
            종류.name, interaction.user.id, item, am))
        button.callback = button_callback
