  ...
  async def inventory_callback(interaction:discord.Interaction):
    ...
    async def equip_callback(interaction:discord.Interaction):
      if inventory.value=="_weapon":
        ...
        cur.execute(f"UPDATE `{interaction.user.id}{inventory.value}` SET wear = 1 WHERE (SELECT @rowid:=@rowid+1 as rowid FROM `{interaction.user.id}{inventory.value}`, (SELECT @rowid:={select.values[0]}))")
